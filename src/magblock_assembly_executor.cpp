 ...
 */
 ...
 planning_scene_diff_.robot_state.joint_state.header.frame_id = "world";
 ...
 RCLCPP_DEBUG(node_->get_logger(), "Moving %s to joint configuration with %zu joints", 
 robot_name.c_str(), joint_values.size());
 }
 ...
 */
 ...
 RCLCPP_INFO(node_->get_logger(), "Simulating %s trajectory execution for %s", 
 segment_name.c_str(), robot_name.c_str());
 ...
 // Interactive prompt before simulation
 prompt("Press 'Next' to simulate " + segment_name);
 draw_title("Simulating " + segment_name);
 
 const auto& joint_trajectory = trajectory.joint_trajectory;
 auto joint_names = joint_trajectory.joint_names;
 
 RCLCPP_INFO(node_->get_logger(), "Simulating trajectory with %zu waypoints", 
 joint_trajectory.points.size());
 
 // Simulate trajectory execution by stepping through waypoints
 ...
 // Calculate sleep time based on trajectory timing and playback speed
 double sleep_time = 0.1; // Default 100ms between waypoints
 if (i > 0) {
 double trajectory_dt = (point.time_from_start.sec + point.time_from_start.nanosec * 1e-9) - 
 (joint_trajectory.points[i-1].time_from_start.sec + 
 joint_trajectory.points[i-1].time_from_start.nanosec * 1e-9);
 sleep_time = trajectory_dt / playback_speed;
 sleep_time = std::max(0.01, std::min(0.5, sleep_time)); // Clamp between 10ms and 500ms
 }
 
 rclcpp::sleep_for(std::chrono::nanoseconds(static_cast<int64_t>(sleep_time * 1e9)));
 
 // Allow ROS to process callbacks
 rclcpp::spin_some(node_);
 }
 
 RCLCPP_INFO(node_->get_logger(), "Completed simulation of %s", segment_name.c_str());
 return true;
 }
 ...
 */
 ...
 RCLCPP_INFO(node_->get_logger(), "Simulating execution of %zu trajectory segments for %s", 
 trajectories.size(), object_name.c_str());
 ...
 RCLCPP_INFO(node_->get_logger(), "Simulating %s segment %zu/%zu for %s", 
 segment_name.c_str(), i+1, trajectories.size(), object_name.c_str());
 ...
 draw_title(segment_name + " Simulation Failed!");
 visual_tools->trigger();
 ...
 // Show completion
 draw_title(segment_name + " Simulation Completed!");
 visual_tools->trigger();
 ...
 draw_title("All Simulation Complete for " + object_name);
 visual_tools->trigger();
 
 RCLCPP_INFO(node_->get_logger(), "Successfully simulated all trajectory segments for %s", 
 object_name.c_str());
 return true;
 }
