...
bool RRTConnect::plan(const skillgraph::State &start, const skillgraph::State &goal,
    ...
    if (robot_id_to_plan >= instance_->getRobotNames().size()) {
    ...
    moveit::core::robotStateToRobotStateMsg(instance_->planning_scene_->getCurrentState(), req.start_state);
    ...
    if (res.error_code_.val == moveit_msgs::MoveItErrorCodes::SUCCESS) {
        ...
        traj.time_from_start.clear();

        const moveit_msgs::RobotTrajectory& moveit_traj_msg = res.trajectory_;
        ...
        for (const auto& point : moveit_traj_msg.joint_trajectory.points) {
            ...
            traj.time_from_start.push_back(point.time_from_start.toSec());
        }
