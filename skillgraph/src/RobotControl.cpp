...
void RobotControl::print_robot_property()
{
...
void RobotControl::set_DH_tool(const std::string fname)
{
    std::cout << "Load DH tool from: " << fname << std::endl;
    DH_tool_ = lego_manipulation::io::LoadMatFromFile(fname);
...
void RobotControl::set_DH_tool_assemble(const std::string fname)
{
    std::cout << "Load DH tool for assemble from: " << fname << std::endl;
    DH_tool_assemble_ = lego_manipulation::io::LoadMatFromFile(fname);
...
void RobotControl::set_DH_tool_disassemble(const std::string fname)
{
    std::cout << "Load DH tool for disassemble from: " << fname << std::endl;
    DH_tool_disassemble_ = lego_manipulation::io::LoadMatFromFile(fname);
...
void RobotControl::Setup(const std::string& DH_fname, const std::string& base_fname)
{
    ...
    DH_ = lego_manipulation::io::LoadMatFromFile(DH_fname);
    
    std::cout << "Load Robot Base from: " << base_fname << std::endl;
    base_frame_ = lego_manipulation::io::LoadMatFromFile(base_fname);
...
void RobotControl::set_JPC_speed(const double& t)
{
...
math::VectorJd RobotControl::JSSA(const math::VectorJd& jerk_ref)
{
...
void RobotControl::computeJacobiDiff(const Eigen::MatrixXd& DH_cur, const Eigen::MatrixXd& qd, const Eigen::MatrixXd& qdd, 
...
void RobotControl::calculateSafetyIdx(const Eigen::MatrixXd& D, const Eigen::MatrixXd& BJ, const double& margin, const Eigen::MatrixXd& u)
{
...
bool RobotControl::reached_goal(math::VectorJd goal)
{
    for(int i=0; i<njoints_; i++)
    {
...
math::VectorJd RobotControl::jpc(const math::VectorJd& goal)
{
...
math::VectorJd RobotControl::pid(const math::VectorJd& goal)
{
    double p_pos = 3;
    double i_pos = 0;
    double d_pos = 0.99;
...
math::VectorJd RobotControl::step(const math::VectorJd& jerk, const math::VectorJd& goal)
{
    Eigen::MatrixXd X(3, 1);
...
void RobotControl::resetPID(int i)
{
    pos_err_sum_(i) = 0;
    vel_err_sum_(i) = 0;
    acc_err_sum_(i) = 0;
