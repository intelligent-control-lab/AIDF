...
namespace tpg {
    ...
    struct PolicyNode : public Node {
        PolicyNode() = default;
        PolicyNode(int robot_id, task::Activity::Type type) : Node(robot_id, type) {
            this->type = type;
        }
    
        task::Activity::Type type;
        bool execute();
        bool isFinished();
    };
    ...
    public:
        ShortcutSamplerADG(const TPGConfig &config, std::shared_ptr<task::ActivityGraph> act_graph,       
    ...
    private:
        ...
        std::shared_ptr<task::ActivityGraph> act_graph_;
        std::vector<std::vector<NodePtr>> intermediate_nodes_;
    };

    class ShortcutIteratorADG : public ShortcutIterator {
    public:
        ShortcutIteratorADG(const TPGConfig &config, std::shared_ptr<task::ActivityGraph> act_graph,
            const std::vector<std::vector<NodePtr>> &intermediate_nodes);
        void init(const std::vector<NodePtr> &start_nodes,
    ...
    public:
        ADG() = default;
        ADG (std::shared_ptr<task::ActivityGraph> activity_graph);

        bool init_from_asynctrajs(std::shared_ptr<env::PlanInstance> instance, const TPGConfig &config, const robot::MRTrajectory &trajectories);
        bool init_from_tpgs(std::shared_ptr<env::PlanInstance> instance, const TPGConfig &config, const std::vector<std::shared_ptr<TPG>> &tpgs);
        ...
        virtual void checkShortcuts(std::shared_ptr<env::PlanInstance> instance, Shortcut &shortcut, const std::vector<std::vector<NodePtr>> &timedNodes) const override;
        ...
        virtual std::shared_ptr<task::ActivityGraph> getActGraph() const override {return act_graph_;}

    private:
        bool addTaskDeps();
        bool findCollisionDeps(std::shared_ptr<env::PlanInstance> instance, const std::vector<std::vector<int>> &reached_t, bool skip_sametask);

        void findCollisionDepsTask(int i, int j, task::ActPtr act_i, task::ActPtr act_j, NodePtr act_i_start_node, NodePtr act_j_start_node, NodePtr act_i_end_node,
                                NodePtr act_j_end_node, std::shared_ptr<env::PlanInstance> instance, const std::vector<std::vector<int>> &reached_t,
                                const std::vector<std::vector<bool>> &visited);
         void findCollisionDepsTaskParallel(int i, int j, task::ActPtr act_i, task::ActPtr act_j, NodePtr act_i_start_node, NodePtr act_j_start_node, NodePtr act_i_end_node,
                                NodePtr act_j_end_node, std::shared_ptr<env::PlanInstance> instance, const std::vector<std::vector<int>> &reached_t,
                                const std::vector<std::vector<bool>> &visited);
        ...
        void updateScene(std::shared_ptr<env::PlanInstance> instance, task::ActPtr act, int existing_robot=-1) const;

        std::shared_ptr<task::ActivityGraph> act_graph_;
        std::shared_ptr<env::MoveitInstance> instance_; // used for moveit execution
        std::vector<std::vector<NodePtr>> intermediate_nodes_;
